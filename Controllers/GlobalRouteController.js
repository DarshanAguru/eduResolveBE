...
export const forgotPassword = async (req, res) => {
  const { query, email, type, phoneNo } = req.body;
  ...
  try {
    ...
    if (query === 'generateOTP') {
      if (type === 'mentor') {
        data = await Mentors.findOne({
          phoneNumber: phoneNo,
          emailId: email
        });
      } else if (type === 'student') {
        data = await Students.findOne({
          phoneNumber: phoneNo,
          emailId: email
        });
      } else if (type === 'teacher') {
        data = await Teachers.findOne({
          phoneNumber: phoneNo,
          emailId: email
        });
      } else if (type === 'organization') {
        data = await LocalAdmins.findOne({
          phoneNumber: phoneNo,
          emailId: email
        });
      } else {
      ...
      const mailOptions = {
        ...
        text:
          `Dear User,\nYour OTP for the EDU RESOLVE account reset ` +
      ...
      const passwordData = await ForgotPassword.findOneAndUpdate(
        { userId: data._id },
        { emailId: data.emailId, type, otp },
      ...
      return res
        .status(200)
        .send({ message: 'OTP sent', userId: data._id });
    }

    if (query === 'verifyOTP&Reset') {
      ...
      if (type === 'mentor') {
        data = await Mentors.findOneAndUpdate(
          { _id: userId },
          { password: hashedPassword }
        );
      } else if (type === 'student') {
        data = await Students.findOneAndUpdate(
          { _id: userId },
          { password: hashedPassword }
        );
      } else if (type === 'teacher') {
        data = await Teachers.findOneAndUpdate(
          { _id: userId },
          { password: hashedPassword }
        );
      } else if (type === 'organization') {
        data = await LocalAdmins.findOneAndUpdate(
          { _id: userId },
          { password: hashedPassword }
        );
      } else {
      ...
      return res
        .status(200)
        .send({ message: 'verified And Reset' });
    }
