...
import GlobalAdminHelper from
  '../globalAdminTests/globalAdminTestHelper.js'
...
describe('/localAdmins endpoints', () => {
  ...
  it('PUT /register fails to register with existing phone number',
    async () => {

      const admin = LocalAdminHelper.localAdminFactory()
      await LocalAdminHelper.registerLocalAdmin(admin)

      const newAdmin = LocalAdminHelper.localAdminFactory({
        name: 'Test Admin 2',
  ...
  it('POST /login logs in the local admin', async () => {
    ...
    const allLocalAdmins = await GlobalAdminHelper
      .getAllLocalAdmins(creds)

    const presLA = allLocalAdmins.body.find(
      val => val.phoneNumber === admin.phoneNumber
    )

    await GlobalAdminHelper.verifyLocalAdmin(presLA._id, creds)

    const res = await LocalAdminHelper.loginLocalAdmin({
  ...
  it('POST /login fails login with non-existent phone number',
    async () => {
  ...
  it('PUT /register fails with invalid phone number', async () => {
  ...
  it('PUT /register fails with invalid email format', async () => {
  ...
  it('POST /logout logs out the local admin', async () => {
    ...
    const loginLA = await LocalAdminHelper.loginLocalAdmin({
      phoneNumber: admin.phoneNumber,
      password: admin.password,
    })
    ...
    const res = await LocalAdminHelper
      .logoutLocalAdmin(credsLA.id, credsLA)
