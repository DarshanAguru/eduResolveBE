...
describe('/globalAdmins endpoints', () => {
  ...
  it('PUT /register fails to register with existing phone number', async () => {
    ...
    const newAdmin = GlobalAdminHelper.globalAdminFactory({ name: 'Test Admin 2', emailId: 'new@example.com' })
  ...
  it('POST /login logs in the global admin', async () => {
    const admin = GlobalAdminHelper.globalAdminFactory({ phoneNumber: '9876543210', name: 'Darshi' })
    await GlobalAdminHelper.registerGlobalAdmin(admin)

    const res = await GlobalAdminHelper.loginGlobalAdmin({ phoneNumber: '9876543210', password: 'Test@123' })
  ...
  it('POST /login fails login with wrong password', async () => {
    ...
    const res = await GlobalAdminHelper.loginGlobalAdmin({ phoneNumber: admin.phoneNumber, password: 'wrong' })
  ...
  it("POST /login fails login with non-existent phone number", async () => {
    const res = await GlobalAdminHelper.loginGlobalAdmin({ phoneNumber: '0000000000', password: 'Pass@123' })
    expect(res.statusCode).toBe(404)
  })  

  it('PUT /register fails to register with missing data', async () => {
    const admin = GlobalAdminHelper.globalAdminFactory({ phoneNumber: '', emailId: '' })
    const res = await GlobalAdminHelper.registerGlobalAdmin(admin)
  ...
  it('PUT /register fails to register with invalid phone number', async () => {
    const admin = GlobalAdminHelper.globalAdminFactory({ phoneNumber: '12345' })
    const res = await GlobalAdminHelper.registerGlobalAdmin(admin)
  ...
  it('PUT /register fails to register with invalid email format', async () => {
    const admin = GlobalAdminHelper.globalAdminFactory({ emailId: 'invalid' })
    const res = await GlobalAdminHelper.registerGlobalAdmin(admin)
  ...
  it('POST /getAllLocalAdmins retrieves all local admins', async () => {
  ...
  it('POST /getAllLocalAdmins fails without credentials', async () => {

    const res = await GlobalAdminHelper.getAllLocalAdmins({id:"", token:""})

    expect(res.statusCode).toBe(401)
    expect(res.body.message).toBe('Not Authorized')
  })

  it('POST /getAllLocalAdmins with no local admins returns empty array', async () => {
    const {loginRes} = await GlobalAdminHelper.createGlobalAdminSession()
    const creds = { id: loginRes.body._id, token: loginRes.body.token }
    const res = await GlobalAdminHelper.getAllLocalAdmins(creds)
...
it('POST /verifyLocalAdmin/:id verifies a local admin', async () => {
    ...
    const creds = { id: loginRes.body._id, token: loginRes.body.token }
    const localAdmin = LocalAdminHelper.localAdminFactory()
    const registerRes = await LocalAdminHelper.registerLocalAdmin(localAdmin)
    const allLocalAdmins = await GlobalAdminHelper.getAllLocalAdmins(creds)
    const localAdminId = allLocalAdmins.body.find(val => val.phoneNumber === localAdmin.phoneNumber)._id
    const res = await GlobalAdminHelper.verifyLocalAdmin(localAdminId, creds)
    expect(res.statusCode).toBe(200)
  ...
  it('POST /rejectLocalAdmin/:id rejects a local admin', async () => {
    ...
    const allLocalAdmins = await GlobalAdminHelper.getAllLocalAdmins(creds)
    const localAdminId = allLocalAdmins.body.find(val => val.phoneNumber === localAdmin.phoneNumber)._id
    const res = await GlobalAdminHelper.rejectLocalAdmin(localAdminId, creds)
    expect(res.statusCode).toBe(200)
  ...
  it('POST /logout logs out the global admin', async () => {
    ...
    const creds = { id: adminId, token: loginRes.body.token }
    const res = await GlobalAdminHelper.logoutGlobalAdmin(adminId, creds)
