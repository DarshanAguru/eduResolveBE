...
describe('/students endpoints', () => {
  ...
  it('PUT /register fails to register with existing phone number', async () => {
    ...
    const newStudent = StudentHelper.studentFactory({ name: 'Test Student 2', emailId: 'new@example.com' })
  ...
  it('POST /login logs in the student', async () => {
    const student = StudentHelper.studentFactory({ phoneNumber: '9999999999', name: 'Darshi' })
    await StudentHelper.registerStudent(student)

    const res = await StudentHelper.loginStudent({ phoneNumber: '9999999999', password: 'Pass@123' })
  ...
  it('POST /login fails login with wrong password', async () => {
    ...
    const res = await StudentHelper.loginStudent({ phoneNumber: student.phoneNumber, password: 'wrong' })
  ...
  it("POST /login fails login with non-existent phone number", async () => {
    const res = await StudentHelper.loginStudent({ phoneNumber: '0000000000', password: 'Pass@123' })
    expect(res.statusCode).toBe(404)
  })  

  it('PUT /register fails to register with missing data', async () => {
    const student = StudentHelper.studentFactory({ phoneNumber: '', emailId: '' })
    const res = await StudentHelper.registerStudent(student)
  ...
  it('PATCH /editDetails edits details of a student', async () => {
    ...
    const updatedDetails = {id: studentId, token: loginRes.body.token, name: 'Updated Name', grade: '11' }
  ...
  it('PATCH /editDetails fails to edit details with invalid token', async () => {
    const {loginRes, registerRes} = await StudentHelper.createStudentSession()
    const studentId = loginRes.body._id
    const updatedDetails = { id: studentId, token: 'invalidtoken', name: 'Updated Name', grade: '11' }
  ...
  it('PATCH /editDetails fails to edit details with invalid id', async () => {
    const {loginRes, registerRes} = await StudentHelper.createStudentSession()
    const studentId = loginRes.body._id
    const updatedDetails = { id: 'invalid', token: loginRes.body.token, name: 'Updated Name', grade: '11' }
